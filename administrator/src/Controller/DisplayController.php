...
final class DisplayController extends BaseController
{
    ...
    private function syncCurrentSportyear():void
    {
        ...
        try{$today=(new \DateTimeImmutable('now',new \DateTimeZone($timezone?:'UTC')))->format('Y-m-d');}
        catch(\Throwable){$today=Factory::getDate()->format('Y-m-d');}

        $q=$db->getQuery(true)->select('id')->from('#__jt_sportyears')
            ->where('published=1')
            ->where('date_start<='.$db->quote($today))
            ->where('date_end>='.$db->quote($today))
            ->order('date_start DESC,id DESC');
        $db->setQuery($q,0,1);
        $id=(int)$db->loadResult();

        $q=$db->getQuery(true)->select('id')->from('#__jt_sportyears')
            ->where('is_current=1')->order('id DESC');
        $db->setQuery($q,0,1);
        $previousId=(int)$db->loadResult();

        if($previousId!==$id){
            $q=$db->getQuery(true)->update('#__jt_sportyears')->set('is_current=0')->where('is_current<>0');
            $db->setQuery($q)->execute();

            if($id>0){
                $q=$db->getQuery(true)->update('#__jt_sportyears')->set('is_current=1')->where('id='.$id);
                $db->setQuery($q)->execute();
            }
        }

        if($id>0){
            (new ClassTransitionService())->applyForSportyear($id,false);
        }
    }
}
