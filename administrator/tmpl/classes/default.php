...
?>
<form action="<?php echo Route::_('index.php?option=com_jugendtraining&view=classes'); ?>" method="post" name="adminForm" id="adminForm">
...
<table class="table table-striped"><thead><tr><th width="1%"><input type="checkbox" name="checkall-toggle" value="" onclick="Joomla.checkAll(this)"></th><th><?php echo Text::_('COM_JUGENDTRAINING_COLUMN_TITLE'); ?></th><th><?php echo Text::_('COM_JUGENDTRAINING_FIELD_CODE'); ?></th><th><?php echo Text::_('COM_JUGENDTRAINING_AGE_RANGE'); ?></th><th><?php echo Text::_('COM_JUGENDTRAINING_COLUMN_STATUS'); ?></th><th>ID</th></tr></thead><tbody>
<?php foreach($this->items as $i=>$item): ?><tr><td><?php echo HTMLHelper::_('grid.id',$i,$item->id); ?></td><td><a href="<?php echo Route::_('index.php?option=com_jugendtraining&task=class.edit&id='.(int)$item->id); ?>"><?php echo htmlspecialchars((string)($item->name),ENT_QUOTES,'UTF-8'); ?></a></td><td><?php echo htmlspecialchars((string)($item->code ?: '–'), ENT_QUOTES, 'UTF-8'); ?></td><td><?php echo htmlspecialchars((string)((($item->min_age ?? '–') . ' – ' . ($item->max_age ?? '–'))), ENT_QUOTES, 'UTF-8'); ?></td><td><?php echo htmlspecialchars((string)($item->published ? Text::_('COM_JUGENDTRAINING_STATUS_PUBLISHED') : Text::_('COM_JUGENDTRAINING_STATUS_UNPUBLISHED')), ENT_QUOTES, 'UTF-8'); ?></td><td><?php echo (int)$item->id; ?></td></tr><?php endforeach; ?>
