...
final class BowsetupModel extends AdminModel {
 ...
 public function getSightSettings():array{$id=(int)Factory::getApplication()->getInput()->getInt('id');if(!$id)return[];$db=$this->getDatabase();$q=$db->getQuery(true)->select('*')->from($db->quoteName('#__jt_sight_settings'))->where('bow_setup_id='.$id)->order('distance_m');$db->setQuery($q);return$db->loadAssocList();}
