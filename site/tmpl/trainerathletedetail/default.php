...
<div class="col-lg-8"><div class="card h-100"><div class="card-header"><h2 class="h4 mb-0"><?php echo Text::_('COM_JUGENDTRAINING_RECENT_ATTENDANCE');?></h2></div><div class="table-responsive"><table class="table mb-0"><thead><tr><th><?php echo Text::_('JDATE');?></th><th><?php echo Text::_('COM_JUGENDTRAINING_TRAINING');?></th><th><?php echo Text::_('JSTATUS');?></th></tr></thead><tbody><?php foreach($this->attendance as$r):?><tr><td><?php echo HTMLHelper::_('date',$r->training_date,Text::_('DATE_FORMAT_LC4'));?></td><td><?php echo htmlspecialchars($r->title,ENT_QUOTES,'UTF-8');?></td><td><?php
$statusKey='COM_JUGENDTRAINING_ATTENDANCE_'.strtoupper((string)$r->status);
$translated=Text::_($statusKey);
echo htmlspecialchars($translated===$statusKey?(string)$r->status:$translated,ENT_QUOTES,'UTF-8');
