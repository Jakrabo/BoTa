  ...
  <div class="col-12 col-md-4">
   <article class="card h-100 text-center">
    <div class="card-body d-flex flex-column align-items-center justify-content-center">
     <?php if($b->badge_image):?><img src="<?php echo htmlspecialchars(Uri::root().ltrim($b->badge_image,'/'),ENT_QUOTES,'UTF-8');?>" alt="" style="width:72px;height:72px;max-width:72px;max-height:72px;object-fit:contain"><?php else:?><span class="jt-badge-fallback" aria-label="<?php echo Text::_('COM_JUGENDTRAINING_ACHIEVEMENT_NO_IMAGE');?>">★</span><?php endif;?>
...
<section class="card"><div class="card-header"><h2 class="h4 mb-0"><?php echo Text::_('COM_JUGENDTRAINING_RECENT_AWARDS');?></h2></div><div class="table-responsive"><table class="table jt-mobile-card-table"><thead><tr><th><?php echo Text::_('COM_JUGENDTRAINING_FIELD_ATHLETE');?></th><th><?php echo Text::_('COM_JUGENDTRAINING_ACHIEVEMENT');?></th><th><?php echo Text::_('COM_JUGENDTRAINING_AWARDED_ON');?></th><th><?php echo Text::_('COM_JUGENDTRAINING_SOURCE');?></th><th><?php echo Text::_('COM_JUGENDTRAINING_COLUMN_ACTIONS');?></th></tr></thead><tbody><?php foreach($c->awards as$a):?><tr><td data-label="<?php echo Text::_('COM_JUGENDTRAINING_FIELD_ATHLETE');?>"><?php echo htmlspecialchars($a->athlete_name,ENT_QUOTES,'UTF-8');?></td><td data-label="<?php echo Text::_('COM_JUGENDTRAINING_ACHIEVEMENT');?>"><div class="d-flex align-items-center gap-2"><?php if(!empty($a->badge_image)):?><img src="<?php echo htmlspecialchars(Uri::root().ltrim($a->badge_image,'/'),ENT_QUOTES,'UTF-8');?>" width="44" height="44" alt=""><?php else:?><span class="jt-badge-fallback jt-badge-fallback--xs" aria-label="<?php echo Text::_('COM_JUGENDTRAINING_ACHIEVEMENT_NO_IMAGE');?>">★</span><?php endif;?><strong><?php echo htmlspecialchars($a->achievement_title,ENT_QUOTES,'UTF-8');?></strong></div></td><td data-label="<?php echo Text::_('COM_JUGENDTRAINING_AWARDED_ON');?>"><?php echo HTMLHelper::_('date',$a->awarded_at,Text::_('DATE_FORMAT_LC4'));?></td><td data-label="<?php echo Text::_('COM_JUGENDTRAINING_SOURCE');?>"><?php echo Text::_($a->award_source==='automatic'?'COM_JUGENDTRAINING_AWARD_AUTOMATIC':'COM_JUGENDTRAINING_AWARD_MANUAL');?></td><td data-label="<?php echo Text::_('COM_JUGENDTRAINING_COLUMN_ACTIONS');?>"><form action="<?php echo Route::_('index.php?option=com_jugendtraining&task=achievement.revoke');?>" method="post" onsubmit="return confirm('<?php echo htmlspecialchars(Text::_('COM_JUGENDTRAINING_CONFIRM_REVOKE_ACHIEVEMENT'),ENT_QUOTES,'UTF-8');?>');"><input type="hidden" name="athlete_id" value="<?php echo(int)$a->athlete_id;?>"><input type="hidden" name="achievement_id" value="<?php echo(int)$a->achievement_id;?>"><button class="btn btn-sm btn-outline-danger" type="submit"><?php echo Text::_('COM_JUGENDTRAINING_REVOKE');?></button><?php echo HTMLHelper::_('form.token');?></form></td></tr><?php endforeach;?></tbody></table></div></section>
